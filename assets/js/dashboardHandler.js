
// creating variables for all screens in dashboard

const homeScreen = document.getElementById("dashboard-home");
const addUsersScreen = document.getElementById("dashboard-add-users");
const addUsersBtn = document.getElementById("addUsersBtn");
const invitationsScreen = document.getElementById("dashboard-invitations")
const feedbacksScreen = document.getElementById("dashboard-feedbacks");

// menu btns

const menuHomeBtn = document.getElementById("menuHomeBtn");
const menuFeedbacksBtn = document.getElementById("menuFeedbacksBtn");
const menuInvitaionsBtn = document.getElementById("menuInvitationsBtn");
const menuAddUsersBtn = document.getElementById("menuAddUsersBtn");
const menuSignOutBtn = document.getElementById("menuSignOutBtn");

// feedback btns

const feedbacksRequestedBtn = document.getElementById("dashboard-feedback-requested-button");
const feedbacksReceivedBtn = document.getElementById("dashboard-feedback-received-button")
const feedbacksAssignBtn = document.getElementById("dashboard-feedback-assign-button");
const feedbacksAssignedBtn = document.getElementById("dashboard-feedback-assigned-button");


let activeScreen;
let dashboardFeedbackScreen;
//object of screens and menu button combinations

const activsScreenBtns = {

    "dashboard-home" : "menuHomeBtn",
    "dashboard-add-users" : "menuAddUsersBtn",
    "dashboard-invitations" : "menuInvitationsBtn",
    "dashboard-feedbacks" : "menuFeedbacksBtn"

}

const feedbackScreenBtns = {
    "assignFeedbacks" : "dashboard-feedback-assign-button",
    "assignedFeedbacks" : "dashboard-feedback-assigned-button",
    "requestedFeedbacks" : "dashboard-feedback-requested-button",
    "receivedFeedbacks" : "dashboard-feedback-received-button"
}

setInitialScreen();
// sets the home screen to visible

function setInitialScreen() {
    // gets active screen from session storage 
    activeScreen = sessionStorage.getItem("activeScreen");
    
    if(activeScreen === null) { // if there was no active screen from the session, sets home screen as active

        homeScreen.classList.add("active")
        setActiveScreen("dashboard-home")
    }
    else { // displays the last active screen
        let screenToShow = document.getElementById(activeScreen); // since some screens are not available for all the users
        // checks if the screen is available
        if(screenToShow == null) { // if it isn't clears active screen
            sessionStorage.clear();
            setInitialScreen();
        }
        else {  // if present displpays active screen
            screenToShow.classList.add('active');
        }
       
    }

}

window.addEventListener('load', () => {
    let activeBtn = document.getElementById(activsScreenBtns[activeScreen]); // gets the apt btn for the active screen from the storage
    if(activeBtn != null) { // if the option is available to the user 
        activeBtn.click();  // simulates a click on the button to display the screen
    }
})

//sets session storage to current screen

function setActiveScreen(screen) {
    sessionStorage.setItem('activeScreen', screen);
}


//hides active screen 

function hideActive() {
    let activeScreen = document.querySelector(".active") //gets active screen
    activeScreen.classList.remove("active")
}

//menu buttons event listeners

// hides all screens and shows home screen when btn is clicked

menuHomeBtn.addEventListener('click', function (e) {
    hideActive();
    homeScreen.classList.add('active');
    setActiveScreen("dashboard-home");
})

//hides all screens and shows add users screen when btn is clicked

if(menuAddUsersBtn !== null) {
    menuAddUsersBtn.addEventListener('click', function(e) {

        hideActive();
        addUsersScreen.classList.add("active"); // sets add users screen as active.
        setActiveScreen("dashboard-add-users");
    })
}


// when user signs out clear the session storage

menuSignOutBtn.addEventListener('click', function () {
    sessionStorage.clear();
    window.location.replace("/profile/signout");
})


// when user clicks on invitaion btn

if(menuInvitaionsBtn !== null) {
    menuInvitaionsBtn.addEventListener('click', async function(e) {

        hideActive(); // hides active screen and sets inivatation screen as active
        invitationsScreen.classList.add('active');
        setActiveScreen("dashboard-invitations");
    
        let userType = e.target.dataset.type;
        let isAdmin = e.target.dataset.admin;
    
        let invitations = await fetch(`profile/getInvitations`).then(res => res.json())
    
        let invitationsContainer = document.querySelector(".dashboard-invitations-container");
        invitationsContainer.innerHTML = "" // clears the invitation container
    
        if(invitations.length == 0) // if there are no invitations
        {
                let inviteContainer = document.createElement('div');

                if(userType == "Employee" && isAdmin == "false") {
                    
                    inviteContainer.classList.add("prompt-img-container");

                    inviteContainer.innerHTML = `

                    <svg class="prompt-img" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" width="797.5" height="834.5" viewBox="0 0 797.5 834.5" xmlns:xlink="http://www.w3.org/1999/xlink"><title>void</title><ellipse cx="308.5" cy="780" rx="308.5" ry="54.5" fill="#3f3d56"/><circle cx="496" cy="301.5" r="301.5" fill="#3f3d56"/><circle cx="496" cy="301.5" r="248.89787" opacity="0.05"/><circle cx="496" cy="301.5" r="203.99362" opacity="0.05"/><circle cx="496" cy="301.5" r="146.25957" opacity="0.05"/><path d="M398.42029,361.23224s-23.70394,66.72221-13.16886,90.42615,27.21564,46.52995,27.21564,46.52995S406.3216,365.62186,398.42029,361.23224Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><path d="M398.42029,361.23224s-23.70394,66.72221-13.16886,90.42615,27.21564,46.52995,27.21564,46.52995S406.3216,365.62186,398.42029,361.23224Z" transform="translate(-201.25 -32.75)" opacity="0.1"/><path d="M415.10084,515.74682s-1.75585,16.68055-2.63377,17.55847.87792,2.63377,0,5.26754-1.75585,6.14547,0,7.02339-9.65716,78.13521-9.65716,78.13521-28.09356,36.8728-16.68055,94.81576l3.51169,58.82089s27.21564,1.75585,27.21564-7.90132c0,0-1.75585-11.413-1.75585-16.68055s4.38962-5.26754,1.75585-7.90131-2.63377-4.38962-2.63377-4.38962,4.38961-3.51169,3.51169-4.38962,7.90131-63.2105,7.90131-63.2105,9.65716-9.65716,9.65716-14.92471v-5.26754s4.38962-11.413,4.38962-12.29093,23.70394-54.43127,23.70394-54.43127l9.65716,38.62864,10.53509,55.3092s5.26754,50.04165,15.80262,69.356c0,0,18.4364,63.21051,18.4364,61.45466s30.72733-6.14547,29.84941-14.04678-18.4364-118.5197-18.4364-118.5197L533.62054,513.991Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><path d="M391.3969,772.97846s-23.70394,46.53-7.90131,48.2858,21.94809,1.75585,28.97148-5.26754c3.83968-3.83968,11.61528-8.99134,17.87566-12.87285a23.117,23.117,0,0,0,10.96893-21.98175c-.463-4.29531-2.06792-7.83444-6.01858-8.16366-10.53508-.87792-22.826-10.53508-22.826-10.53508Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><path d="M522.20753,807.21748s-23.70394,46.53-7.90131,48.28581,21.94809,1.75584,28.97148-5.26754c3.83968-3.83969,11.61528-8.99134,17.87566-12.87285a23.117,23.117,0,0,0,10.96893-21.98175c-.463-4.29531-2.06792-7.83444-6.01857-8.16367-10.53509-.87792-22.826-10.53508-22.826-10.53508Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><circle cx="295.90488" cy="215.43252" r="36.90462" fill="#ffb8b8"/><path d="M473.43048,260.30832S447.07,308.81154,444.9612,308.81154,492.41,324.62781,492.41,324.62781s13.70743-46.39439,15.81626-50.61206Z" transform="translate(-201.25 -32.75)" fill="#ffb8b8"/><path d="M513.86726,313.3854s-52.67543-28.97148-57.943-28.09356-61.45466,50.04166-60.57673,70.2339,7.90131,53.55335,7.90131,53.55335,2.63377,93.05991,7.90131,93.93783-.87792,16.68055.87793,16.68055,122.90931,0,123.78724-2.63377S513.86726,313.3854,513.86726,313.3854Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><path d="M543.2777,521.89228s16.68055,50.91958,2.63377,49.16373-20.19224-43.89619-20.19224-43.89619Z" transform="translate(-201.25 -32.75)" fill="#ffb8b8"/><path d="M498.50359,310.31267s-32.48318,7.02339-27.21563,50.91957,14.9247,87.79237,14.9247,87.79237l32.48318,71.11182,3.51169,13.16886,23.70394-6.14547L528.353,425.32067s-6.14547-108.86253-14.04678-112.37423A33.99966,33.99966,0,0,0,498.50359,310.31267Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><polygon points="277.5 414.958 317.885 486.947 283.86 411.09 277.5 414.958" opacity="0.1"/><path d="M533.896,237.31585l.122-2.82012,5.6101,1.39632a6.26971,6.26971,0,0,0-2.5138-4.61513l5.97581-.33413a64.47667,64.47667,0,0,0-43.1245-26.65136c-12.92583-1.87346-27.31837.83756-36.182,10.43045-4.29926,4.653-7.00067,10.57018-8.92232,16.60685-3.53926,11.11821-4.26038,24.3719,3.11964,33.40938,7.5006,9.18513,20.602,10.98439,32.40592,12.12114,4.15328.4,8.50581.77216,12.35457-.83928a29.721,29.721,0,0,0-1.6539-13.03688,8.68665,8.68665,0,0,1-.87879-4.15246c.5247-3.51164,5.20884-4.39635,8.72762-3.9219s7.74984,1.20031,10.062-1.49432c1.59261-1.85609,1.49867-4.559,1.70967-6.99575C521.28248,239.785,533.83587,238.70653,533.896,237.31585Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><circle cx="559" cy="744.5" r="43" fill="#6c63ff"/><circle cx="54" cy="729.5" r="43" fill="#6c63ff"/><circle cx="54" cy="672.5" r="31" fill="#6c63ff"/><circle cx="54" cy="624.5" r="22" fill="#6c63ff"/></svg>
                    <h1 class="prompt-text">Looks Empty</h1>
                    <p class="prompt-subtext"> You have no invitations </p>

                    `
                
                    invitationsContainer.appendChild(inviteContainer)
                }
                else {
                
                    inviteContainer.classList.add("prompt-img-container");

                    inviteContainer.innerHTML = `

                    <svg class="prompt-img" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" width="797.5" height="834.5" viewBox="0 0 797.5 834.5" xmlns:xlink="http://www.w3.org/1999/xlink"><title>void</title><ellipse cx="308.5" cy="780" rx="308.5" ry="54.5" fill="#3f3d56"/><circle cx="496" cy="301.5" r="301.5" fill="#3f3d56"/><circle cx="496" cy="301.5" r="248.89787" opacity="0.05"/><circle cx="496" cy="301.5" r="203.99362" opacity="0.05"/><circle cx="496" cy="301.5" r="146.25957" opacity="0.05"/><path d="M398.42029,361.23224s-23.70394,66.72221-13.16886,90.42615,27.21564,46.52995,27.21564,46.52995S406.3216,365.62186,398.42029,361.23224Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><path d="M398.42029,361.23224s-23.70394,66.72221-13.16886,90.42615,27.21564,46.52995,27.21564,46.52995S406.3216,365.62186,398.42029,361.23224Z" transform="translate(-201.25 -32.75)" opacity="0.1"/><path d="M415.10084,515.74682s-1.75585,16.68055-2.63377,17.55847.87792,2.63377,0,5.26754-1.75585,6.14547,0,7.02339-9.65716,78.13521-9.65716,78.13521-28.09356,36.8728-16.68055,94.81576l3.51169,58.82089s27.21564,1.75585,27.21564-7.90132c0,0-1.75585-11.413-1.75585-16.68055s4.38962-5.26754,1.75585-7.90131-2.63377-4.38962-2.63377-4.38962,4.38961-3.51169,3.51169-4.38962,7.90131-63.2105,7.90131-63.2105,9.65716-9.65716,9.65716-14.92471v-5.26754s4.38962-11.413,4.38962-12.29093,23.70394-54.43127,23.70394-54.43127l9.65716,38.62864,10.53509,55.3092s5.26754,50.04165,15.80262,69.356c0,0,18.4364,63.21051,18.4364,61.45466s30.72733-6.14547,29.84941-14.04678-18.4364-118.5197-18.4364-118.5197L533.62054,513.991Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><path d="M391.3969,772.97846s-23.70394,46.53-7.90131,48.2858,21.94809,1.75585,28.97148-5.26754c3.83968-3.83968,11.61528-8.99134,17.87566-12.87285a23.117,23.117,0,0,0,10.96893-21.98175c-.463-4.29531-2.06792-7.83444-6.01858-8.16366-10.53508-.87792-22.826-10.53508-22.826-10.53508Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><path d="M522.20753,807.21748s-23.70394,46.53-7.90131,48.28581,21.94809,1.75584,28.97148-5.26754c3.83968-3.83969,11.61528-8.99134,17.87566-12.87285a23.117,23.117,0,0,0,10.96893-21.98175c-.463-4.29531-2.06792-7.83444-6.01857-8.16367-10.53509-.87792-22.826-10.53508-22.826-10.53508Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><circle cx="295.90488" cy="215.43252" r="36.90462" fill="#ffb8b8"/><path d="M473.43048,260.30832S447.07,308.81154,444.9612,308.81154,492.41,324.62781,492.41,324.62781s13.70743-46.39439,15.81626-50.61206Z" transform="translate(-201.25 -32.75)" fill="#ffb8b8"/><path d="M513.86726,313.3854s-52.67543-28.97148-57.943-28.09356-61.45466,50.04166-60.57673,70.2339,7.90131,53.55335,7.90131,53.55335,2.63377,93.05991,7.90131,93.93783-.87792,16.68055.87793,16.68055,122.90931,0,123.78724-2.63377S513.86726,313.3854,513.86726,313.3854Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><path d="M543.2777,521.89228s16.68055,50.91958,2.63377,49.16373-20.19224-43.89619-20.19224-43.89619Z" transform="translate(-201.25 -32.75)" fill="#ffb8b8"/><path d="M498.50359,310.31267s-32.48318,7.02339-27.21563,50.91957,14.9247,87.79237,14.9247,87.79237l32.48318,71.11182,3.51169,13.16886,23.70394-6.14547L528.353,425.32067s-6.14547-108.86253-14.04678-112.37423A33.99966,33.99966,0,0,0,498.50359,310.31267Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><polygon points="277.5 414.958 317.885 486.947 283.86 411.09 277.5 414.958" opacity="0.1"/><path d="M533.896,237.31585l.122-2.82012,5.6101,1.39632a6.26971,6.26971,0,0,0-2.5138-4.61513l5.97581-.33413a64.47667,64.47667,0,0,0-43.1245-26.65136c-12.92583-1.87346-27.31837.83756-36.182,10.43045-4.29926,4.653-7.00067,10.57018-8.92232,16.60685-3.53926,11.11821-4.26038,24.3719,3.11964,33.40938,7.5006,9.18513,20.602,10.98439,32.40592,12.12114,4.15328.4,8.50581.77216,12.35457-.83928a29.721,29.721,0,0,0-1.6539-13.03688,8.68665,8.68665,0,0,1-.87879-4.15246c.5247-3.51164,5.20884-4.39635,8.72762-3.9219s7.74984,1.20031,10.062-1.49432c1.59261-1.85609,1.49867-4.559,1.70967-6.99575C521.28248,239.785,533.83587,238.70653,533.896,237.31585Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><circle cx="559" cy="744.5" r="43" fill="#6c63ff"/><circle cx="54" cy="729.5" r="43" fill="#6c63ff"/><circle cx="54" cy="672.5" r="31" fill="#6c63ff"/><circle cx="54" cy="624.5" r="22" fill="#6c63ff"/></svg>
                    <h1 class="prompt-text">Looks Empty</h1>
                    <p class="prompt-subtext"> You haven't invitated anyone yet! </p>

                    `

                    invitationsContainer.appendChild(inviteContainer)
                }

        }
        else { // if invitations are present

            if(userType == "Employee" && isAdmin == "false") {
    
                for(let invite of invitations) { // adds the invitation sent by the organizations to the DOM
        
                    let inviteContainer = document.createElement('div')
                    inviteContainer.classList.add('inviteContainer')
        
                    inviteContainer.innerHTML = `
        
                    <div class="invite-status-container">
                        <span class="invite-status-indicator"></span>
                        <p class="invite-status-text">${invite.status}</p>
                    </div>
                    <div class="invite-info"> 
                        <p><strong>Company : </strong> ${invite.company.name}</p>
                        <p><strong>Position : </strong> ${invite.position}</p>
                        <p><strong>Invited By : </strong>  ${invite.invitationBy.name}</p>
                    </div>
                    <div class="invite-button-container">
                        <button class="invite-btn invite-accept-button" data-inviteId=${invite._id} data-type="accept" ><i class="invite-icons fa-solid fa-check"></i> Accept</button>
                        <button class="invite-btn invite-reject-button" data-inviteId=${invite._id} data-type="reject" ><i class="invite-icons fa-solid fa-xmark"> </i>Reject</button>
                    </div>                
        
                    `
                    inviteContainer.setAttribute('data-inviteContainerId', invite._id);
        
                    invitationsContainer.appendChild(inviteContainer)
        
                } 
        
        
            
            } 
            
            else {
                
                // if the user is an admin or an organization - lists all the inivtations sent by the user as cards
                
                for(let invite of invitations) {
        
                    let inviteContainer = document.createElement('table');
                    inviteContainer.classList.add('inviteContainer');
                    

                    inviteContainer.innerHTML = `
                    
                        <tr colspan="2" class="invite-status-container">
                            <td colspan="2" class="invite-status" >${invite.status}</td>
                        </tr>
                        <tr>
                            <th>Email</th>
                            <td>${invite.email}</td>
                        </tr>
                        <tr>
                            <th>Position</th>
                            <td>${invite.position}</td>
                        </tr>
                        <tr>
                            <th>Is Admin</th>
                            <td>${invite.isAdmin}</td>
                        </tr>
                        <tr>
                            <th>Invited By</th>
                            <td>${invite.invitationBy.name}</td>
                        </tr>

                    


                    `
        
                    inviteContainer.setAttribute('data-id', invite._id);
        
                    invitationsContainer.appendChild(inviteContainer)
        
                }
            }
        }
    
    })
}


//when user accepts or rejects invitation

window.addEventListener('click', (e) => {
    if(e.target.classList.contains("invite-btn")) {
        let id = e.target.dataset.inviteid;
        let invitation = document.querySelector(`[data-invitecontainerid = "${id}"]`);
        invitation.style.display = "none";
        if(e.target.dataset.type == "accept") {

            fetch(`/profile/acceptInvitation?invite=${id}`, {
                method : "POST",
            })
            .then((res) => {
                window.location.replace('/');
            })

        }
        else if(e.target.dataset.type == "reject") {

            fetch(`/profile/rejectInvitation?invite=${id}`, {
                method : "POST",
            })
            .then((res) => window.location.reload());

        }
    }
});


//feedbacks screen
if(menuFeedbacksBtn != null) {
    
    menuFeedbacksBtn.addEventListener('click', (e) => {
        
        hideActive();
        feedbacksScreen.classList.add("active"); // sets feedbacks screen as active.
        setActiveScreen("dashboard-feedbacks");
        setActiveDashboardScreen(e);
        
    });
}

//since we have multiple screens in feedbacks sections catering to different users this function sets the active feedback screen in the dashboard
// so that users gets to the same screen if they reload the page 

function setActiveDashboardScreen(e) {

    // setting the feedback screens in dashboard
    dashboardFeedbackScreen = sessionStorage.getItem('dashboardFeedbackScreen');

    let userType = e.target.dataset.type;
    let isAdmin = e.target.dataset.admin;

    if(dashboardFeedbackScreen == null) { // if there is no active screen from the session storage
    
        if(userType == "Employee" && isAdmin == "false")
        {
            sessionStorage.setItem("dashboardFeedbackScreen", "requestedFeedbacks"); // setting default feedback section screens for the user
            setDashboardScreen(e);
        }
        else {
            sessionStorage.setItem("dashboardFeedbackScreen", "assignFeedbacks");
            setDashboardScreen(e);
        }

    }
    else { // displays the screen 
        setDashboardScreen(e);
    }
    

}

// sets feedback screen on dashboard

function setDashboardScreen(e) {

    let feedbackScreen = sessionStorage.getItem("dashboardFeedbackScreen");
    let feedbackBtn = document.getElementById(feedbackScreenBtns[feedbackScreen]);
    if(feedbackBtn == null) { // if the active screen in the session is not available for the user , resets the session storage and loads the default screen
        sessionStorage.removeItem('dashboardFeedbackScreen');
        setActiveDashboardScreen(e);
    }
    else { // if present displays the screen
            feedbackBtn.click();
    }

}

//-------------------- assign feedbacks --------------------

// since assigning feedbacks is only available for the admins or org, we need to check if the function is available to the user

if(feedbacksAssignBtn != null) {  // if the function is available loads the assign screen
    
    feedbacksAssignBtn.addEventListener('click', async (e) => {
        styleActiveFeedbackBtns(e);
        sessionStorage.setItem("dashboardFeedbackScreen", "assignFeedbacks");
        let employees = await fetch('/profile/getEmployees').then((res) => res.json());
        let feedbacksContainer = document.querySelector(".dashboard-feedbacks-container");
        feedbacksContainer.innerHTML = ""
        if(employees.length < 2) { // if there are less than two employees shows a prompt

            let imageContainer = document.createElement('div');
            imageContainer.classList.add("prompt-img-container");

            imageContainer.innerHTML = `

                <svg class="prompt-img" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" width="691.33587" height="489.02997" viewBox="0 0 691.33587 489.02997" xmlns:xlink="http://www.w3.org/1999/xlink"><path d="M942.37829,365.12258c-9.53656,33.08356-37.38213,56.2424-66.42859,75.91553q-6.0835,4.12119-12.17087,8.05276c-.02749.012-.05582.03768-.08365.05-.19525.12542-.39088.2511-.5728.37734-.83757.5399-1.67544,1.08016-2.50606,1.61384l.45626.205s.448.23823-.04214.03819c-.14589-.061-.29872-.11582-.44431-.17719-16.87415-6.80561-34.048-14.11275-46.97949-26.8619-13.41453-13.23916-21.25393-34.00418-13.99008-51.39659a33.20766,33.20766,0,0,1,3.5956-6.45359c.5892-.84313,1.22378-1.65009,1.88447-2.4421A35.59319,35.59319,0,0,1,866.229,374.85964c2.134-19.86884-14.24941-36.512-30.016-48.79213-15.77356-12.274-33.69674-25.64058-36.74619-45.39912-1.70294-10.9934,2.11992-21.64706,9.08376-30.19945.2156-.25986.43089-.51936.653-.7721a51.945,51.945,0,0,1,33.387-17.91151c24.18672-2.51372,47.6804,10.28875,65.25152,27.10539C936.12263,285.94754,953.21951,327.50678,942.37829,365.12258Z" transform="translate(-254.33206 -205.48502)" fill="#f2f2f2"/><path d="M896.916,321.84967a88.30333,88.30333,0,0,1,10.57914,23.333,76.67467,76.67467,0,0,1,2.65029,22.64563c-.43468,15.70057-5.26563,31.043-12.86659,44.708a141.38984,141.38984,0,0,1-21.32914,28.50182q-6.0835,4.12119-12.17087,8.05276c-.02749.012-.05582.03768-.08365.05-.19525.12542-.39088.2511-.5728.37734-.83757.5399-1.67544,1.08016-2.50606,1.61384,0,0,.90429.4432.41412.24316-.14589-.061-.29872-.11582-.44431-.17719a78.15741,78.15741,0,0,0-25.0093-67.29586A78.82691,78.82691,0,0,0,803.21216,366.486c.5892-.84313,1.22378-1.65009,1.88447-2.4421a81.72794,81.72794,0,0,1,13.49027,5.10487,80.08109,80.08109,0,0,1,36.20832,34.909,81.87421,81.87421,0,0,1,8.91443,44.92834c.69331-.62828,1.38747-1.2702,2.06069-1.90606,12.82253-11.95459,24.2115-25.67651,31.91919-41.51276a91.39288,91.39288,0,0,0,9.57005-43.241c-.62905-16.3343-7.13721-31.46406-16.56016-44.63539-10.09394-14.09647-22.74877-26.5681-36.26251-37.3723a217.37693,217.37693,0,0,0-45.09985-27.84376,1.56289,1.56289,0,0,1-.78653-2.00584,1.32935,1.32935,0,0,1,.653-.7721,1.15723,1.15723,0,0,1,1.005.0427c1.98939.91584,3.965,1.8442,5.92469,2.8127a219.84437,219.84437,0,0,1,45.42128,29.96187C874.92422,293.91268,887.51108,306.93552,896.916,321.84967Z" transform="translate(-254.33206 -205.48502)" fill="#fff"/><circle cx="532.41146" cy="313.69784" r="35.37135" fill="#2f2e41"/><ellipse cx="751.37218" cy="492.34942" rx="14.63642" ry="10.97732" transform="translate(-382.40386 470.02115) rotate(-45)" fill="#2f2e41"/><ellipse cx="810.15005" cy="483.12936" rx="10.97732" ry="14.63642" transform="translate(-206.72186 832.8831) rotate(-66.86956)" fill="#2f2e41"/><path d="M861.65189,671.055a131.81283,131.81283,0,0,1-19.19971,11.07l-.99023-1.75-9.48-16.63995-.83008-1.46-4.84961-8.51-.01025-.01-15.31006-26.9-7.86963-13.81,23.36963-16.39,9.09033,19.22,4.92969,10.44,20.29,42.91Z" transform="translate(-254.33206 -205.48502)" fill="#a0616a"/><path d="M840.772,606.155l-2.93018-4.59-12.96-20.33A16.35623,16.35623,0,0,0,794.982,594.505l8.01025,28.54a5.87464,5.87464,0,0,0,7.98975,3.81c.03027-.01.05029-.02.08007-.03l24.51026-10.95,2.63965-1.18a5.90107,5.90107,0,0,0,2.56006-8.54Z" transform="translate(-254.33206 -205.48502)" fill="#6c63ff"/><path d="M770.35208,606.795l-6.81982,15.4-1.48,3.35-25.25,57.04-.81006,1.83a131.09328,131.09328,0,0,1-19.68018-10.24l.68018-1.92,20.60986-58.15,5.06006-14.28,11.08008,2.79Z" transform="translate(-254.33206 -205.48502)" fill="#a0616a"/><circle cx="787.86534" cy="527.62681" r="30.01929" transform="translate(-307.34259 760.37645) rotate(-61.33681)" fill="#a0616a"/><path d="M840.19193,613.245a66.79891,66.79891,0,0,0-2.3501-11.68,65.095,65.095,0,0,0-18.80957-29.84,17.78783,17.78783,0,0,0-11.90039-4.55h-42.29a17.78318,17.78318,0,0,0-15.92969,25.78l4.83008,9.66,9.79,19.58,2.34961,4.69,10.47021,20.95.83008,1.66,53.1499,14.62.23-.51c.2002-.45.39991-.89.58985-1.33.29-.67.58008-1.32.8501-1.98,5.36035-12.71,7.89013-24.02,8.5-33.98A75.64747,75.64747,0,0,0,840.19193,613.245Z" transform="translate(-254.33206 -205.48502)" fill="#6c63ff"/><path d="M772.51224,572.655a16.33208,16.33208,0,0,0-21.68994,7.8l-3.77,5.69-12.6001,19.02a5.89309,5.89309,0,0,0,2.41016,8.59l.73974.35,24.4502,11.44,1.73975.81a5.65793,5.65793,0,0,0,2.08984.53,5.76186,5.76186,0,0,0,2.82031-.49,5.87867,5.87867,0,0,0,3.23-3.68l8.46-28.31A16.37189,16.37189,0,0,0,772.51224,572.655Z" transform="translate(-254.33206 -205.48502)" fill="#6c63ff"/><path d="M832.002,660.295a5.4613,5.4613,0,0,0-.8501-1.73,22.88259,22.88259,0,0,0-4.84961-4.8l-.01025-.01c-6.42969-4.87-19.23-10.56-43.91993-10.71h-.04A5.8972,5.8972,0,0,0,776.962,646.495l-.60987,1.34-3.93017,8.64a4.50764,4.50764,0,0,1-1.52979,1.85c-3.55029,2.51-14.85009,11.61-20.68994,28.99-.21.62-.41015,1.25-.6001,1.9a132.09857,132.09857,0,0,0,76.37012-.69l.54981-2.27,5.46-22.51995.09033-.37006A5.91045,5.91045,0,0,0,832.002,660.295Z" transform="translate(-254.33206 -205.48502)" fill="#2f2e41"/><path d="M756.32205,508.74476a40.82856,40.82856,0,0,0,23.33326,7.211,25.01983,25.01983,0,0,1-9.91672,4.07982,82.328,82.328,0,0,0,33.62847.18895,21.76455,21.76455,0,0,0,7.03979-2.41786,8.90912,8.90912,0,0,0,4.34529-5.81136c.7378-4.21489-2.54649-8.04421-5.95961-10.625a43.96047,43.96047,0,0,0-36.94134-7.3819c-4.12658,1.06667-8.26049,2.8689-10.94082,6.18286s-3.47428,8.423-.92061,11.83553Z" transform="translate(-254.33206 -205.48502)" fill="#2f2e41"/><rect x="1.08599" y="30.47938" width="542.31608" height="2.17361" fill="#3f3d56"/><circle cx="19.56169" cy="16.30746" r="6.52083" fill="#3f3d56"/><circle cx="38.30909" cy="16.30746" r="6.52083" fill="#3f3d56"/><circle cx="57.05649" cy="16.30746" r="6.52083" fill="#3f3d56"/><path d="M680.28324,362.107H372.48438a6.97561,6.97561,0,1,1,0-13.95122H680.28324a6.97561,6.97561,0,1,1,0,13.95122Z" transform="translate(-254.33206 -205.48502)" fill="#ccc"/><path d="M680.28324,437.09483H372.48438a6.97561,6.97561,0,1,1,0-13.95122H680.28324a6.97561,6.97561,0,1,1,0,13.95122Z" transform="translate(-254.33206 -205.48502)" fill="#e6e6e6"/><path d="M680.28324,465.86923H372.48438a6.97561,6.97561,0,1,1,0-13.95122H680.28324a6.97561,6.97561,0,1,1,0,13.95122Z" transform="translate(-254.33206 -205.48502)" fill="#e6e6e6"/><path d="M630.146,332.46066H422.62159a6.97561,6.97561,0,1,1,0-13.95122H630.146a6.97561,6.97561,0,0,1,0,13.95122Z" transform="translate(-254.33206 -205.48502)" fill="#ccc"/><path d="M480.99771,408.32044h-108.122a6.97562,6.97562,0,1,1,0-13.95123h108.122a6.97561,6.97561,0,0,1,0,13.95123Z" transform="translate(-254.33206 -205.48502)" fill="#e6e6e6"/><path d="M680.67458,408.32044h-108.122a6.97562,6.97562,0,1,1,0-13.95123h108.122a6.97562,6.97562,0,1,1,0,13.95123Z" transform="translate(-254.33206 -205.48502)" fill="#e6e6e6"/><path d="M791.212,205.485H261.93216a7.61168,7.61168,0,0,0-7.6001,7.61v358.65a7.6096,7.6096,0,0,0,7.6001,7.6h395.79c-.1001-.72-.18017-1.45-.26025-2.17H261.93216a5.435,5.435,0,0,1-5.43017-5.43V213.095a5.44356,5.44356,0,0,1,5.43017-5.44H791.212a5.44563,5.44563,0,0,1,5.43994,5.44v219.81c.72021.05,1.45019.11,2.17041.18V213.095A7.61385,7.61385,0,0,0,791.212,205.485Z" transform="translate(-254.33206 -205.48502)" fill="#3f3d56"/><path d="M657.7222,579.345c-.1001-.72-.18017-1.45-.26025-2.17-.02-.11-.02979-.21-.04-.32q-.43506-3.945-.62988-7.97-.82471,3.465-1.48975,6.96c.02978.34.06006.67.09961,1.01.01025.11.02.21.04.32.08008.72.16015,1.45.26025,2.17.27979,2.23.62012,4.43,1.02,6.62.00977-.09.02-.17.02979-.26.28027-2.11.60009-4.21.98-6.31A.092.092,0,0,1,657.7222,579.345Zm141.1001-148.27c-.72022-.07-1.4502-.13-2.17041-.18-.10986-.01-.21-.01-.31983-.02-3.20019-.24-6.43994-.36-9.70019-.36a132.0411,132.0411,0,0,0-131.32959,145.33c.02978.34.06006.67.09961,1.01.01025.11.02.21.04.32.08008.72.16015,1.45.26025,2.17.27979,2.23.62012,4.43,1.02,6.62a132.57434,132.57434,0,0,0,79.27,98.45c2.08008.86,4.17969,1.68,6.31006,2.43,1.3999.51,2.7998.98,4.21973,1.43,1.02.33,2.05029.64,3.08007.94,1.46.43,2.93995.84,4.43018,1.22a131.35519,131.35519,0,0,0,107.61963-19.38h.01025a131.92892,131.92892,0,0,0,56.96973-108.54C918.63187,493.835,865.91214,437.245,798.8223,431.075ZM860.792,669.225a129.37688,129.37688,0,0,1-19.33008,11.15,128.00858,128.00858,0,0,1-14.93994,5.88,130.22913,130.22913,0,0,1-71.08008,2.47q-2.63965-.645-5.23975-1.41-2.56494-.75-5.08008-1.59c-1.37011-.47-2.74023-.95-4.10009-1.46-1.41993-.54-2.81983-1.1-4.21973-1.68A130.33475,130.33475,0,0,1,657.732,579.395a.092.092,0,0,1-.00977-.05c-.1001-.72-.18017-1.45-.26025-2.17-.02-.11-.02979-.21-.04-.32q-.43506-3.945-.62988-7.97-.16481-3.165-.16016-6.37a130.14245,130.14245,0,0,1,130-130c3.26025,0,6.5.12,9.70019.36.10987.01.21.02.31983.03.72021.05,1.45019.11,2.17041.18,65.98974,6.16,117.80957,61.85,117.80957,129.43A129.91362,129.91362,0,0,1,860.792,669.225Zm-119.77,15.04c-1.41993-.54-2.81983-1.1-4.21973-1.68A130.33475,130.33475,0,0,1,657.732,579.395c-.37989,2.1-.69971,4.2-.98,6.31-.00977.09-.02.17-.02979.26a132.57434,132.57434,0,0,0,79.27,98.45c2.08008.86,4.17969,1.68,6.31006,2.43C741.86234,685.995,741.43216,685.135,741.022,684.265Zm55.31-253.39c-3.20019-.24-6.43994-.36-9.70019-.36a132.0411,132.0411,0,0,0-131.32959,145.33c.02978.34.06006.67.09961,1.01.01025.11.02.21.04.32h2.02c-.02-.11-.02979-.21-.04-.32q-.43506-3.945-.62988-7.97-.16481-3.165-.16016-6.37a130.14245,130.14245,0,0,1,130-130c3.26025,0,6.5.12,9.70019.36.10987.01.21.02.31983.03v-2.01C796.542,430.885,796.44193,430.885,796.33206,430.875Z" transform="translate(-254.33206 -205.48502)" fill="#3f3d56"/><path d="M893.92661,464.30169c40.544.6924,40.53765,61.00085-.0013,61.68733C853.38261,525.29667,853.389,464.98822,893.92661,464.30169Z" transform="translate(-254.33206 -205.48502)" fill="#6c63ff"/><path d="M893.92679,511.93982a2.17023,2.17023,0,0,1-2.16765-2.16764V498.27924a.96717.96717,0,0,0-.96591-.9662H879.3003a2.16794,2.16794,0,0,1,0-4.33588h11.49293a.96667.96667,0,0,0,.96591-.96562V480.518a2.1675,2.1675,0,0,1,4.335,0v11.49352a.96667.96667,0,0,0,.96591.96562H908.553a2.16794,2.16794,0,0,1,0,4.33588H897.06005a.96717.96717,0,0,0-.96591.9662v11.49294A2.17017,2.17017,0,0,1,893.92679,511.93982Z" transform="translate(-254.33206 -205.48502)" fill="#fff"/></svg>
                <h1 class="prompt-text">You can't assign feedbacks yet!!</h1>
                <p class="prompt-subtext"> You need atleast 3 users to start assigning feedbacks </p>

            `

            feedbacksContainer.appendChild(imageContainer)

        }
        else { // adds the assign feedback form to the DOM
            

                let assignFormContainer = document.createElement('div');
                assignFormContainer.classList.add('flex-c')

                assignFormContainer.innerHTML = `

                    <form action="/profile/assignFeedback" method="post" id="assignFeedbackForm">

                            <label for="feedbackFor" class="credentials-label">Feedback for: </label>
                            <select name="feedbackFor" id="feedbackFor" class="credentials_input" required>
                                <option value="" disabled selected>Select Employee</option>
                            </select>
                            <label for="assignedTo" class="credentials-label">Assigned to:</label>
                            <select name="assignedTo" id="assignedTo" class="credentials_input" required>
                                <option value="" disabled selected>Select Employee</option>
                            </select>
                            <button class="primaryBtn">Assign</button>

                    </form>

                `

                await feedbacksContainer.appendChild(assignFormContainer);

                let feedbacksfor = document.getElementById("feedbackFor");
                let assignedto = document.getElementById("assignedTo");

                for(let employee of employees) {

                    let option = document.createElement('option');
                    option.text = employee.name;
                    option.value = employee._id;

                    feedbacksfor.appendChild(option);
                    

                }

                for(let employee of employees) {

                    let option = document.createElement('option');
                    option.text = employee.name;
                    option.value = employee._id;
                    
                    assignedto.appendChild(option);

                }


            }
        
        
    });
}

// ------------  asigned Feedbacks  ---------------------

// checking if the function is available for the user

if(feedbacksAssignedBtn != null) { // if it is available -- render screen to the DOM 
    feedbacksAssignedBtn.addEventListener('click', async (e) => {
        styleActiveFeedbackBtns(e);
        sessionStorage.setItem("dashboardFeedbackScreen", "assignedFeedbacks");
        let feedbacksContainer = document.querySelector(".dashboard-feedbacks-container");
        feedbacksContainer.innerHTML = ""

        let feedbacks = await fetch('/profile/assignedFeedbacks').then(res => res.json());

        if(feedbacks.length > 0) {
            for(let feedback of feedbacks) {
                
                let feedbackContainer = document.createElement('table')
                feedbackContainer.innerHTML = `

                    <tr>
                        <th>Assigned to</th>
                        <td>${feedback.reviewer.name}</td>
                    </tr>
                    <tr>
                        <th>Reviewee</th>
                        <td>${feedback.reviewee.name}</td>
                    </tr>
                    <tr>
                        <th>Status</th>
                        <td>${feedback.status}</td>
                    </tr>

                `
                feedbacksContainer.appendChild(feedbackContainer);
            }
        }
        else { // if not render a prompt

            let imageContainer = document.createElement('div');
            imageContainer.classList.add("prompt-img-container");

            imageContainer.innerHTML = `

                <svg class="prompt-img" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" width="797.5" height="834.5" viewBox="0 0 797.5 834.5" xmlns:xlink="http://www.w3.org/1999/xlink"><title>void</title><ellipse cx="308.5" cy="780" rx="308.5" ry="54.5" fill="#3f3d56"/><circle cx="496" cy="301.5" r="301.5" fill="#3f3d56"/><circle cx="496" cy="301.5" r="248.89787" opacity="0.05"/><circle cx="496" cy="301.5" r="203.99362" opacity="0.05"/><circle cx="496" cy="301.5" r="146.25957" opacity="0.05"/><path d="M398.42029,361.23224s-23.70394,66.72221-13.16886,90.42615,27.21564,46.52995,27.21564,46.52995S406.3216,365.62186,398.42029,361.23224Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><path d="M398.42029,361.23224s-23.70394,66.72221-13.16886,90.42615,27.21564,46.52995,27.21564,46.52995S406.3216,365.62186,398.42029,361.23224Z" transform="translate(-201.25 -32.75)" opacity="0.1"/><path d="M415.10084,515.74682s-1.75585,16.68055-2.63377,17.55847.87792,2.63377,0,5.26754-1.75585,6.14547,0,7.02339-9.65716,78.13521-9.65716,78.13521-28.09356,36.8728-16.68055,94.81576l3.51169,58.82089s27.21564,1.75585,27.21564-7.90132c0,0-1.75585-11.413-1.75585-16.68055s4.38962-5.26754,1.75585-7.90131-2.63377-4.38962-2.63377-4.38962,4.38961-3.51169,3.51169-4.38962,7.90131-63.2105,7.90131-63.2105,9.65716-9.65716,9.65716-14.92471v-5.26754s4.38962-11.413,4.38962-12.29093,23.70394-54.43127,23.70394-54.43127l9.65716,38.62864,10.53509,55.3092s5.26754,50.04165,15.80262,69.356c0,0,18.4364,63.21051,18.4364,61.45466s30.72733-6.14547,29.84941-14.04678-18.4364-118.5197-18.4364-118.5197L533.62054,513.991Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><path d="M391.3969,772.97846s-23.70394,46.53-7.90131,48.2858,21.94809,1.75585,28.97148-5.26754c3.83968-3.83968,11.61528-8.99134,17.87566-12.87285a23.117,23.117,0,0,0,10.96893-21.98175c-.463-4.29531-2.06792-7.83444-6.01858-8.16366-10.53508-.87792-22.826-10.53508-22.826-10.53508Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><path d="M522.20753,807.21748s-23.70394,46.53-7.90131,48.28581,21.94809,1.75584,28.97148-5.26754c3.83968-3.83969,11.61528-8.99134,17.87566-12.87285a23.117,23.117,0,0,0,10.96893-21.98175c-.463-4.29531-2.06792-7.83444-6.01857-8.16367-10.53509-.87792-22.826-10.53508-22.826-10.53508Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><circle cx="295.90488" cy="215.43252" r="36.90462" fill="#ffb8b8"/><path d="M473.43048,260.30832S447.07,308.81154,444.9612,308.81154,492.41,324.62781,492.41,324.62781s13.70743-46.39439,15.81626-50.61206Z" transform="translate(-201.25 -32.75)" fill="#ffb8b8"/><path d="M513.86726,313.3854s-52.67543-28.97148-57.943-28.09356-61.45466,50.04166-60.57673,70.2339,7.90131,53.55335,7.90131,53.55335,2.63377,93.05991,7.90131,93.93783-.87792,16.68055.87793,16.68055,122.90931,0,123.78724-2.63377S513.86726,313.3854,513.86726,313.3854Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><path d="M543.2777,521.89228s16.68055,50.91958,2.63377,49.16373-20.19224-43.89619-20.19224-43.89619Z" transform="translate(-201.25 -32.75)" fill="#ffb8b8"/><path d="M498.50359,310.31267s-32.48318,7.02339-27.21563,50.91957,14.9247,87.79237,14.9247,87.79237l32.48318,71.11182,3.51169,13.16886,23.70394-6.14547L528.353,425.32067s-6.14547-108.86253-14.04678-112.37423A33.99966,33.99966,0,0,0,498.50359,310.31267Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><polygon points="277.5 414.958 317.885 486.947 283.86 411.09 277.5 414.958" opacity="0.1"/><path d="M533.896,237.31585l.122-2.82012,5.6101,1.39632a6.26971,6.26971,0,0,0-2.5138-4.61513l5.97581-.33413a64.47667,64.47667,0,0,0-43.1245-26.65136c-12.92583-1.87346-27.31837.83756-36.182,10.43045-4.29926,4.653-7.00067,10.57018-8.92232,16.60685-3.53926,11.11821-4.26038,24.3719,3.11964,33.40938,7.5006,9.18513,20.602,10.98439,32.40592,12.12114,4.15328.4,8.50581.77216,12.35457-.83928a29.721,29.721,0,0,0-1.6539-13.03688,8.68665,8.68665,0,0,1-.87879-4.15246c.5247-3.51164,5.20884-4.39635,8.72762-3.9219s7.74984,1.20031,10.062-1.49432c1.59261-1.85609,1.49867-4.559,1.70967-6.99575C521.28248,239.785,533.83587,238.70653,533.896,237.31585Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><circle cx="559" cy="744.5" r="43" fill="#6c63ff"/><circle cx="54" cy="729.5" r="43" fill="#6c63ff"/><circle cx="54" cy="672.5" r="31" fill="#6c63ff"/><circle cx="54" cy="624.5" r="22" fill="#6c63ff"/></svg>
                <h1 class="prompt-text">You haven't assigned any feedbacks</h1>
                <p class="prompt-subtext"> Start Assgining now!! </p>

            `

            feedbacksContainer.appendChild(imageContainer)

        }
        
    })
}


// ------------- Received Feedbacks ------------------

// since received feedbacks is only available for the employees - we need to check if the functionality is available

if(feedbacksReceivedBtn != null) { // if it is available -- add feedbacks to DOM

    feedbacksReceivedBtn.addEventListener('click', async (e) => {
        styleActiveFeedbackBtns(e);
        sessionStorage.setItem("dashboardFeedbackScreen", "receivedFeedbacks");
        let feedbacksContainer = document.querySelector(".dashboard-feedbacks-container");
        feedbacksContainer.innerHTML = ""

        let feedbacks = await fetch('/profile/receivedFeedbacks').then((res) => res.json());
        
        if(feedbacks.length > 0) {

            for(let feedback of feedbacks) {
                
                let feedbackContainer = document.createElement('table')
                feedbackContainer.innerHTML = `

                    <tr>
                        <th>Reviewd By</th>
                        <td>${feedback.reviewer.name}</td>
                    </tr>
                    <tr>
                        <th>Assigned By</th>
                        <td>${feedback.assignedBy.name}</td>
                    </tr>
                    <tr>
                        <th>Rating</th>
                        <td>${feedback.rating}</td>
                    </tr>
                    
                    

                `

                if(feedback.feedback != "") {
                    console.log(feedback.feedback)
                    feedbackContainer.insertAdjacentHTML("beforeend",
                    `
                    <tr>
                        <th colspan="2">Feedback</th>
                    </tr>
                    <tr>
                        <td colspan="2">${feedback.feedback}<td>
                    </tr>`)
                }
                feedbacksContainer.appendChild(feedbackContainer);

            }

        }
        else { // if itsn't show prompt

            let imageContainer = document.createElement('div');
            imageContainer.classList.add("prompt-img-container");

            imageContainer.innerHTML = `

                <svg class="prompt-img" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" width="797.5" height="834.5" viewBox="0 0 797.5 834.5" xmlns:xlink="http://www.w3.org/1999/xlink"><title>void</title><ellipse cx="308.5" cy="780" rx="308.5" ry="54.5" fill="#3f3d56"/><circle cx="496" cy="301.5" r="301.5" fill="#3f3d56"/><circle cx="496" cy="301.5" r="248.89787" opacity="0.05"/><circle cx="496" cy="301.5" r="203.99362" opacity="0.05"/><circle cx="496" cy="301.5" r="146.25957" opacity="0.05"/><path d="M398.42029,361.23224s-23.70394,66.72221-13.16886,90.42615,27.21564,46.52995,27.21564,46.52995S406.3216,365.62186,398.42029,361.23224Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><path d="M398.42029,361.23224s-23.70394,66.72221-13.16886,90.42615,27.21564,46.52995,27.21564,46.52995S406.3216,365.62186,398.42029,361.23224Z" transform="translate(-201.25 -32.75)" opacity="0.1"/><path d="M415.10084,515.74682s-1.75585,16.68055-2.63377,17.55847.87792,2.63377,0,5.26754-1.75585,6.14547,0,7.02339-9.65716,78.13521-9.65716,78.13521-28.09356,36.8728-16.68055,94.81576l3.51169,58.82089s27.21564,1.75585,27.21564-7.90132c0,0-1.75585-11.413-1.75585-16.68055s4.38962-5.26754,1.75585-7.90131-2.63377-4.38962-2.63377-4.38962,4.38961-3.51169,3.51169-4.38962,7.90131-63.2105,7.90131-63.2105,9.65716-9.65716,9.65716-14.92471v-5.26754s4.38962-11.413,4.38962-12.29093,23.70394-54.43127,23.70394-54.43127l9.65716,38.62864,10.53509,55.3092s5.26754,50.04165,15.80262,69.356c0,0,18.4364,63.21051,18.4364,61.45466s30.72733-6.14547,29.84941-14.04678-18.4364-118.5197-18.4364-118.5197L533.62054,513.991Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><path d="M391.3969,772.97846s-23.70394,46.53-7.90131,48.2858,21.94809,1.75585,28.97148-5.26754c3.83968-3.83968,11.61528-8.99134,17.87566-12.87285a23.117,23.117,0,0,0,10.96893-21.98175c-.463-4.29531-2.06792-7.83444-6.01858-8.16366-10.53508-.87792-22.826-10.53508-22.826-10.53508Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><path d="M522.20753,807.21748s-23.70394,46.53-7.90131,48.28581,21.94809,1.75584,28.97148-5.26754c3.83968-3.83969,11.61528-8.99134,17.87566-12.87285a23.117,23.117,0,0,0,10.96893-21.98175c-.463-4.29531-2.06792-7.83444-6.01857-8.16367-10.53509-.87792-22.826-10.53508-22.826-10.53508Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><circle cx="295.90488" cy="215.43252" r="36.90462" fill="#ffb8b8"/><path d="M473.43048,260.30832S447.07,308.81154,444.9612,308.81154,492.41,324.62781,492.41,324.62781s13.70743-46.39439,15.81626-50.61206Z" transform="translate(-201.25 -32.75)" fill="#ffb8b8"/><path d="M513.86726,313.3854s-52.67543-28.97148-57.943-28.09356-61.45466,50.04166-60.57673,70.2339,7.90131,53.55335,7.90131,53.55335,2.63377,93.05991,7.90131,93.93783-.87792,16.68055.87793,16.68055,122.90931,0,123.78724-2.63377S513.86726,313.3854,513.86726,313.3854Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><path d="M543.2777,521.89228s16.68055,50.91958,2.63377,49.16373-20.19224-43.89619-20.19224-43.89619Z" transform="translate(-201.25 -32.75)" fill="#ffb8b8"/><path d="M498.50359,310.31267s-32.48318,7.02339-27.21563,50.91957,14.9247,87.79237,14.9247,87.79237l32.48318,71.11182,3.51169,13.16886,23.70394-6.14547L528.353,425.32067s-6.14547-108.86253-14.04678-112.37423A33.99966,33.99966,0,0,0,498.50359,310.31267Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><polygon points="277.5 414.958 317.885 486.947 283.86 411.09 277.5 414.958" opacity="0.1"/><path d="M533.896,237.31585l.122-2.82012,5.6101,1.39632a6.26971,6.26971,0,0,0-2.5138-4.61513l5.97581-.33413a64.47667,64.47667,0,0,0-43.1245-26.65136c-12.92583-1.87346-27.31837.83756-36.182,10.43045-4.29926,4.653-7.00067,10.57018-8.92232,16.60685-3.53926,11.11821-4.26038,24.3719,3.11964,33.40938,7.5006,9.18513,20.602,10.98439,32.40592,12.12114,4.15328.4,8.50581.77216,12.35457-.83928a29.721,29.721,0,0,0-1.6539-13.03688,8.68665,8.68665,0,0,1-.87879-4.15246c.5247-3.51164,5.20884-4.39635,8.72762-3.9219s7.74984,1.20031,10.062-1.49432c1.59261-1.85609,1.49867-4.559,1.70967-6.99575C521.28248,239.785,533.83587,238.70653,533.896,237.31585Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><circle cx="559" cy="744.5" r="43" fill="#6c63ff"/><circle cx="54" cy="729.5" r="43" fill="#6c63ff"/><circle cx="54" cy="672.5" r="31" fill="#6c63ff"/><circle cx="54" cy="624.5" r="22" fill="#6c63ff"/></svg>
                <h1 class="prompt-text">Looks Empty</h1>
                <p class="prompt-subtext"> You haven't received any feedbacks yet </p>

            `

            feedbacksContainer.appendChild(imageContainer)
        }

    })
}

// --------------- Requested Feedbacks -----------------

// check if the funciton is available for the user 

if(feedbacksRequestedBtn != null) { // if available - add the form to DOM

    feedbacksRequestedBtn.addEventListener('click', async (e) => {
        styleActiveFeedbackBtns(e);
        sessionStorage.setItem("dashboardFeedbackScreen", "requestedFeedbacks");
        let feedbacksContainer = document.querySelector(".dashboard-feedbacks-container");
        feedbacksContainer.innerHTML = "";

        let feedback = await fetch('/profile/getRequestedFeedbacks').then((res) => res.json());
        

        if(feedback != null) { // if there are requested feedbacks for the user
            
                let feedbackForm = document.createElement("form");
                feedbackForm.classList.add("flex-c")
                feedbackForm.action = `/profile/submitFeedback?id=${feedback._id}`;
                feedbackForm.method = "post"
                feedbackForm.innerHTML = `

                        <p><strong>Review for : </strong> ${feedback.reviewee.name}</p>
                        <p><strong>Assigned by : </strong> ${feedback.assignedBy.name}</p>
                        <label for="feedbackRating">Rating</label>
                        <input  name="feedbackRating" type="number" min="1" max="5" id="feedbackRating" class="credentials_input" required>
                        <label for="feedbackDescription">Feedback</label>
                        <textarea name="feedbackDescription" id="feedbackDescription" class="credentials_input" row="5" col="33" placeholder="Feedback(Optional)"></textarea>
                        <button class="primaryBtn" type="submit">Submit Feedback</button>
                `
                feedbacksContainer.appendChild(feedbackForm)
            
        }
        else {
            // add empty prompt to DOM

            let imageContainer = document.createElement('div');
            imageContainer.classList.add("prompt-img-container");

            imageContainer.innerHTML = `

                <svg class="prompt-img" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" width="797.5" height="834.5" viewBox="0 0 797.5 834.5" xmlns:xlink="http://www.w3.org/1999/xlink"><title>void</title><ellipse cx="308.5" cy="780" rx="308.5" ry="54.5" fill="#3f3d56"/><circle cx="496" cy="301.5" r="301.5" fill="#3f3d56"/><circle cx="496" cy="301.5" r="248.89787" opacity="0.05"/><circle cx="496" cy="301.5" r="203.99362" opacity="0.05"/><circle cx="496" cy="301.5" r="146.25957" opacity="0.05"/><path d="M398.42029,361.23224s-23.70394,66.72221-13.16886,90.42615,27.21564,46.52995,27.21564,46.52995S406.3216,365.62186,398.42029,361.23224Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><path d="M398.42029,361.23224s-23.70394,66.72221-13.16886,90.42615,27.21564,46.52995,27.21564,46.52995S406.3216,365.62186,398.42029,361.23224Z" transform="translate(-201.25 -32.75)" opacity="0.1"/><path d="M415.10084,515.74682s-1.75585,16.68055-2.63377,17.55847.87792,2.63377,0,5.26754-1.75585,6.14547,0,7.02339-9.65716,78.13521-9.65716,78.13521-28.09356,36.8728-16.68055,94.81576l3.51169,58.82089s27.21564,1.75585,27.21564-7.90132c0,0-1.75585-11.413-1.75585-16.68055s4.38962-5.26754,1.75585-7.90131-2.63377-4.38962-2.63377-4.38962,4.38961-3.51169,3.51169-4.38962,7.90131-63.2105,7.90131-63.2105,9.65716-9.65716,9.65716-14.92471v-5.26754s4.38962-11.413,4.38962-12.29093,23.70394-54.43127,23.70394-54.43127l9.65716,38.62864,10.53509,55.3092s5.26754,50.04165,15.80262,69.356c0,0,18.4364,63.21051,18.4364,61.45466s30.72733-6.14547,29.84941-14.04678-18.4364-118.5197-18.4364-118.5197L533.62054,513.991Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><path d="M391.3969,772.97846s-23.70394,46.53-7.90131,48.2858,21.94809,1.75585,28.97148-5.26754c3.83968-3.83968,11.61528-8.99134,17.87566-12.87285a23.117,23.117,0,0,0,10.96893-21.98175c-.463-4.29531-2.06792-7.83444-6.01858-8.16366-10.53508-.87792-22.826-10.53508-22.826-10.53508Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><path d="M522.20753,807.21748s-23.70394,46.53-7.90131,48.28581,21.94809,1.75584,28.97148-5.26754c3.83968-3.83969,11.61528-8.99134,17.87566-12.87285a23.117,23.117,0,0,0,10.96893-21.98175c-.463-4.29531-2.06792-7.83444-6.01857-8.16367-10.53509-.87792-22.826-10.53508-22.826-10.53508Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><circle cx="295.90488" cy="215.43252" r="36.90462" fill="#ffb8b8"/><path d="M473.43048,260.30832S447.07,308.81154,444.9612,308.81154,492.41,324.62781,492.41,324.62781s13.70743-46.39439,15.81626-50.61206Z" transform="translate(-201.25 -32.75)" fill="#ffb8b8"/><path d="M513.86726,313.3854s-52.67543-28.97148-57.943-28.09356-61.45466,50.04166-60.57673,70.2339,7.90131,53.55335,7.90131,53.55335,2.63377,93.05991,7.90131,93.93783-.87792,16.68055.87793,16.68055,122.90931,0,123.78724-2.63377S513.86726,313.3854,513.86726,313.3854Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><path d="M543.2777,521.89228s16.68055,50.91958,2.63377,49.16373-20.19224-43.89619-20.19224-43.89619Z" transform="translate(-201.25 -32.75)" fill="#ffb8b8"/><path d="M498.50359,310.31267s-32.48318,7.02339-27.21563,50.91957,14.9247,87.79237,14.9247,87.79237l32.48318,71.11182,3.51169,13.16886,23.70394-6.14547L528.353,425.32067s-6.14547-108.86253-14.04678-112.37423A33.99966,33.99966,0,0,0,498.50359,310.31267Z" transform="translate(-201.25 -32.75)" fill="#d0cde1"/><polygon points="277.5 414.958 317.885 486.947 283.86 411.09 277.5 414.958" opacity="0.1"/><path d="M533.896,237.31585l.122-2.82012,5.6101,1.39632a6.26971,6.26971,0,0,0-2.5138-4.61513l5.97581-.33413a64.47667,64.47667,0,0,0-43.1245-26.65136c-12.92583-1.87346-27.31837.83756-36.182,10.43045-4.29926,4.653-7.00067,10.57018-8.92232,16.60685-3.53926,11.11821-4.26038,24.3719,3.11964,33.40938,7.5006,9.18513,20.602,10.98439,32.40592,12.12114,4.15328.4,8.50581.77216,12.35457-.83928a29.721,29.721,0,0,0-1.6539-13.03688,8.68665,8.68665,0,0,1-.87879-4.15246c.5247-3.51164,5.20884-4.39635,8.72762-3.9219s7.74984,1.20031,10.062-1.49432c1.59261-1.85609,1.49867-4.559,1.70967-6.99575C521.28248,239.785,533.83587,238.70653,533.896,237.31585Z" transform="translate(-201.25 -32.75)" fill="#2f2e41"/><circle cx="559" cy="744.5" r="43" fill="#6c63ff"/><circle cx="54" cy="729.5" r="43" fill="#6c63ff"/><circle cx="54" cy="672.5" r="31" fill="#6c63ff"/><circle cx="54" cy="624.5" r="22" fill="#6c63ff"/></svg>
                <h1 class="prompt-text">Looks Empty</h1>
                <p class="prompt-subtext"> You don't have any requested feedbacks </p>

            `

            feedbacksContainer.appendChild(imageContainer)

        }

    });
}



// sets the active feedback color to purple

function styleActiveFeedbackBtns(e) {
    let activeFeedbackBtn = document.querySelector(".dashboard-feedback-btns.menu-active");
    if(activeFeedbackBtn != null) {
        activeFeedbackBtn.classList.remove("menu-active");
        e.target.classList.add("menu-active")
    }
    else {
        e.target.classList.add('menu-active')
    }
}


